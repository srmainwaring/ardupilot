...
bool AP_Arming_Copter::arm_checks(AP_Arming::Method method)
{
    ...
    if (check_enabled(ARMING_CHECK_RC)) {
        ...
        if (!((method == AP_Arming::Method::MAVLINK || method == AP_Arming::Method::SCRIPTING || method == AP_Arming::Method::DDS) && (copter.flightmode->mode_number() == Mode::Number::GUIDED || copter.flightmode->mode_number() == Mode::Number::GUIDED_NOGPS || copter.flightmode->mode_number() == Mode::Number::AUTO))) {
...
bool AP_Arming_Copter::disarm(const AP_Arming::Method method, bool do_disarm_checks)
{
    ...
    if (do_disarm_checks &&
        (method == AP_Arming::Method::MAVLINK || method == AP_Arming::Method::DDS) &&
