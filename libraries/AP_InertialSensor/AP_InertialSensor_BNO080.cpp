 ...
 */
#ifndef BNO080_EXT_SYNC_ENABLE
#define BNO080_EXT_SYNC_ENABLE 0
#endif

// code to debug unexpected register changes
#define BNO080_DEBUG_REG_CHANGE 0

#if BNO080_DEBUG_REG_CHANGE
#include <GCS_MAVLink/GCS.h>
#endif

#include "AP_InertialSensor_BNO080_registers.h"
...
AP_InertialSensor_BNO080::~AP_InertialSensor_BNO080()
{
...
AP_InertialSensor_Backend *AP_InertialSensor_BNO080::probe(AP_InertialSensor &imu,
                                                               ...
                                                               enum Rotation rotation)
{
    ...
    AP_InertialSensor_BNO080 *sensor =
        new AP_InertialSensor_BNO080(imu, std::move(dev), rotation);
...
//     sensor = new AP_InertialSensor_BNO080(imu, std::move(dev), rotation);
//     if (!sensor || !sensor->_init()) {
//         delete sensor;
//         return nullptr;
//     }
...
{
#ifdef BNO080_DRDY_PIN
    _drdy_pin = hal.gpio->channel(BNO080_DRDY_PIN);
...
void AP_InertialSensor_BNO080::_fifo_reset(bool log_error)
{
...
void AP_InertialSensor_BNO080::_fast_fifo_reset()
{
    fast_reset_count++;
...
bool AP_InertialSensor_BNO080::_has_auxiliary_bus()
{
...
void AP_InertialSensor_BNO080::start()
{
    ...
    gdev = DEVTYPE_INS_BNO080;
    adev = DEVTYPE_INS_BNO080;
    ...
    _dev->register_periodic_callback(1000000UL / _gyro_backend_rate_hz, FUNCTOR_BIND_MEMBER(&AP_InertialSensor_BNO080::_poll_data, void));
}

// get a startup banner to output to the GCS
bool AP_InertialSensor_BNO080::get_output_banner(char* banner, uint8_t banner_len) {
    if (_fast_sampling) {
 ...
 */
bool AP_InertialSensor_BNO080::update() /* front end */
{
 ...
 */
void AP_InertialSensor_BNO080::accumulate()
{
    // nothing to do
}

AuxiliaryBus *AP_InertialSensor_BNO080::get_auxiliary_bus()
{
    if (_auxiliary_bus) {
    ...
    if (_has_auxiliary_bus()) {
        _auxiliary_bus = new AP_BNO080_AuxiliaryBus(*this, _dev->get_bus_id());
    }
 ...
 * status register.
 */
bool AP_InertialSensor_BNO080::_data_ready()
{
    if (_drdy_pin) {
 ...
 */
void AP_InertialSensor_BNO080::_poll_data()
{
    _read_fifo();
...
*/
void AP_InertialSensor_BNO080::_check_register_change(void)
{
    if (_mpu_type != Invensense_ICM20602) {
        return;
    }
...
bool AP_InertialSensor_BNO080::_accumulate(uint8_t *samples, uint8_t n_samples)
{
 ...
 */
bool AP_InertialSensor_BNO080::_accumulate_sensor_rate_sampling(uint8_t *samples, uint8_t n_samples)
{
    int32_t tsum = 0;
...
void AP_InertialSensor_BNO080::_read_fifo()
{
    uint8_t n_samples;
    uint16_t bytes_read;
...
*/
bool AP_InertialSensor_BNO080::_check_raw_temp(int16_t t2)
{
...
bool AP_InertialSensor_BNO080::_block_read(uint8_t reg, uint8_t *buf,
                                            uint32_t size)
{
    return _dev->read_registers(reg, buf, size);
}

uint8_t AP_InertialSensor_BNO080::_register_read(uint8_t reg)
{
    uint8_t val = 0;
...
void AP_InertialSensor_BNO080::_register_write(uint8_t reg, uint8_t val, bool checked)
{
 ...
 */
void AP_InertialSensor_BNO080::_set_filter_register(void)
{
    uint8_t config;
 ...
 */
bool AP_InertialSensor_BNO080::_check_whoami(void)
{
...
bool AP_InertialSensor_BNO080::_hardware_init(void)
{
    WITH_SEMAPHORE(_dev->get_semaphore());
...
AP_BNO080_AuxiliaryBusSlave::AP_BNO080_AuxiliaryBusSlave(AuxiliaryBus &bus, uint8_t addr,
                                                         uint8_t instance)
...
int AP_BNO080_AuxiliaryBusSlave::_set_passthrough(uint8_t reg, uint8_t size,
                                                  uint8_t *out)
{
    auto &backend = AP_InertialSensor_BNO080::from(_bus.get_backend());
    uint8_t addr;
...
int AP_BNO080_AuxiliaryBusSlave::passthrough_read(uint8_t reg, uint8_t *buf,
                                                   uint8_t size)
{
    if (_registered) {
...
int AP_BNO080_AuxiliaryBusSlave::passthrough_write(uint8_t reg, uint8_t val)
{
    if (_registered) {
        DEV_PRINTF("Error: can't passthrough when slave is already configured\n");
        return -1;
...
int AP_BNO080_AuxiliaryBusSlave::read(uint8_t *buf)
{
    if (!_registered) {
...
AP_BNO080_AuxiliaryBus::AP_BNO080_AuxiliaryBus(AP_InertialSensor_BNO080 &backend, uint32_t devid)
    : AuxiliaryBus(backend, 4, devid)
{
}

AP_HAL::Semaphore *AP_BNO080_AuxiliaryBus::get_semaphore()
{
    return static_cast<AP_InertialSensor_BNO080&>(_ins_backend)._dev->get_semaphore();
}

AuxiliaryBusSlave *AP_BNO080_AuxiliaryBus::_instantiate_slave(uint8_t addr, uint8_t instance)
{
    ...
    return new AP_BNO080_AuxiliaryBusSlave(*this, addr, instance);
}

void AP_BNO080_AuxiliaryBus::_configure_slaves()
{
    auto &backend = AP_InertialSensor_BNO080::from(_ins_backend);

    // if (backend._mpu_type == AP_InertialSensor_BNO080::Invensense_ICM20789) {
...
int AP_BNO080_AuxiliaryBus::_configure_periodic_read(AuxiliaryBusSlave *slave,
                                                     uint8_t reg, uint8_t size)
{
    ...
    AP_BNO080_AuxiliaryBusSlave *mpu_slave =
        static_cast<AP_BNO080_AuxiliaryBusSlave*>(slave);
...
AP_HAL::Device::PeriodicHandle AP_BNO080_AuxiliaryBus::register_periodic_callback(uint32_t period_usec, AP_HAL::Device::PeriodicCb cb)
{
    auto &backend = AP_InertialSensor_BNO080::from(_ins_backend);
